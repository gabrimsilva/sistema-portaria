<?php
/**
 * View: Formulário de Edição de Pré-Cadastro de Visitante
 * 
 * @version 2.0.0
 * @var array $cadastro Dados do cadastro carregados pelo controller
 */

$pageTitle = 'Editar Pré-Cadastro - Visitante';
...
document.addEventListener('DOMContentLoaded', function() {
    var photoCapture = new PhotoCapture('visitante-photo-capture-container');
        ...
        if (capturedImage) {
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    fetch('/pre-cadastros/visitantes?action=upload_foto', {
                        method: 'POST',
                        body: photoData
                    })
                    .then(function() {
                        window.location.href = '/pre-cadastros/visitantes?success=updated';
                    })
                    .catch(function() {
                        window.location.href = '/pre-cadastros/visitantes?success=updated&photo_warning=1';
                    });
                } else {
