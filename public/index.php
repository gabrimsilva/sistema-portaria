...
// ✅ EXCEÇÃO: Permitir acesso a fotos autorizadas
// 🔒 VALIDAÇÃO CANÔNICA: Prevenir traversal (../, %2e%2e, etc)
$isAllowedUpload = false;
$allowedUploadDirs = [
    '/uploads/profissionais/',
    '/uploads/visitantes/',
    '/uploads/prestadores/'
];

foreach ($allowedUploadDirs as $allowedDir) {
    if (strpos($pathParsed, $allowedDir) !== false) {
        $dirRelative = ltrim($allowedDir, '/');
        $requestedFile = __DIR__ . $pathParsed;
        $canonicalPath = realpath($requestedFile);
        $allowedBase = realpath(__DIR__ . '/' . rtrim($dirRelative, '/'));
...
if ((strpos($requestUri, '/uploads/') !== false || strpos($requestUri, 'uploads/') !== false) && !$isAllowedUpload) {
...
try {
    ...
    switch ($path) {
        ...
        case 'api/pre-cadastros/prestadores/stats':
            require_once '../src/controllers/PreCadastrosPrestadoresController.php';
            $controller = new PreCadastrosPrestadoresController();
            $controller->statsJson();
            break;
        ...
        case 'api/pre-cadastros/visitantes/search-documento':
            require_once '../src/controllers/PreCadastrosVisitantesController.php';
            $controller = new PreCadastrosVisitantesController();
            $controller->searchByDocumento();
            break;
        
        case 'api/pre-cadastros/prestadores/search-documento':
            require_once '../src/controllers/PreCadastrosPrestadoresController.php';
            $controller = new PreCadastrosPrestadoresController();
            $controller->searchByDocumento();
            break;
        
        case 'api/profissionais-renner/search-documento':
            require_once '../src/controllers/ProfissionaisRennerController.php';
            $controller = new ProfissionaisRennerController();
            $controller->searchByDocumento();
            break;
        ...
        default:
            ...
            // Handle new API endpoints
                ...
                if (strpos($path, 'uploads/') === 0 && 
                    strpos($path, 'uploads/profissionais/') !== 0 &&
                    strpos($path, 'uploads/visitantes/') !== 0 &&
                    strpos($path, 'uploads/prestadores/') !== 0) {
                    http_response_code(403);
