...
- **Photo Capture System** (October 27, 2025): Implemented browser-based photo capture for Visitantes and Prestadores pré-cadastros. Features: reusable PhotoCapture component with webcam access, photo preview/retake UI, upload endpoints (/visitantes?action=upload_foto, /prestadores-servico?action=upload_foto), local file storage in /public/uploads/visitantes/ and /public/uploads/prestadores/, foto_url field in cadastro tables, optional capture (doesn't block registration flow), and global helper function window.uploadPhotoIfNeeded() for integration. Security: 2MB limit, MIME type validation (JPG/PNG/WEBP), automatic cleanup of old photos, CSRF protection.
